...
class ViolationsTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $index_gravite=0;
        for($i=1;$i<=12;$i++) {
            $violations['individuelle'][] = array('nom_violation'=>trans('violations.violation_individuelle_'.$i), 'type_violation_id'=>$type_violation_id, 'gravite_id'=>$gravites_individuelle[$index_gravite], 'created_at' => Carbon::now()->format('Y-m-d H:i:s'));
            $index_gravite++;
        }
        ...
        $index_gravite=0;
        for($i=1;$i<=10;$i++) {
            $violations['massives'][] = array('nom_violation'=>trans('violations.violation_massives_'.$i), 'type_violation_id'=>$type_violation_id, 'gravite_id'=>$gravites_massives[$index_gravite], 'created_at' => Carbon::now()->format('Y-m-d H:i:s'));
            $index_gravite++;
        }
